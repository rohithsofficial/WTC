...
const StaffQRScannerScreen = () => {
  ...
  useEffect(() => {
    ...
    const getStaffInfo = async () => {
      try {
        if (auth.currentUser) {
          const staffDocRef = db.collection('users').doc(auth.currentUser.uid);
  ...
  const handleBillSubmit = async () => {
    ...
    setLoading(true);
    try {
      const userDocRef = db.collection('users').doc(scannedUserId);
      const userDoc = await userDocRef.get();
  ...
  const createOfflineOrder = async (preview: TransactionPreview): Promise<string> => {
    try {
      const currentTimestamp = firestore.FieldValue.serverTimestamp() as any;
      const orderId = generateReadableOrderId();
      const autoGeneratedId = db.collection('orders').doc().id;
  ...
  const createLoyaltyTransaction = async (preview: TransactionPreview, orderId: string) => {
    try {
      ...
      await db.collection('loyaltyTransactions').add(loyaltyTransactionData);
    } catch (error) {
  ...
  const updateCustomerProfile = async (preview: TransactionPreview) => {
    try {
      ...
      await userDocRef.update({
        ...
        totalSpent: firestore.FieldValue.increment(preview.customerPays),
