...
const MenuScreen = () => {
  ...
  const loadCategoriesAndProducts = async () => {
    try {
      ...
      setProducts(allProducts.map(prod => ({
        id: prod.id,
        name: prod.name || '',
        ...
        imagelink_square: prod.imagelink_square || '',
        special_ingredient: prod.special_ingredient || '',
        featured: prod.featured || false,
        type: prod.type || '',
        description: prod.description || '',
        average_rating: prod.average_rating || 0,
        ratings_count: prod.ratings_count || 0,
        createdAt: prod.createdAt || new Date().toISOString(),
        updatedAt: prod.updatedAt || new Date().toISOString(),
        roasted: prod.roasted || '',
  ...
  const getProductPrice = (product: Product) => {
    if (product.prices && product.prices.length > 0) {
      // Find the first valid price object with a numeric price > 0
  ...
  const filteredProducts = products.filter((p) => {
    ...
    const matchesSearch =
      ...
      (p.special_ingredient &&
        p.special_ingredient.toLowerCase().includes(searchLower)) ||
      // All ingredients
      (p.ingredients &&
  ...
  const getFeaturedProducts = () => {
    ...
    const featuredProducts = products.filter(
      (product) => product.featured === true && getProductPrice(product) > 0
    );
  ...
  const renderCategoryChip = (category: Category) => (
    ...
    >
      <MaterialIcons
        name={category.icon as any || "local-cafe"}
        size={20}
        color={
    ...
    >
      <Image
        ...
        onError={() => {
          // Optional: Handle image load error
          console.log(
            "Failed to load featured image for product:",
            product.name
          );
        }}
      />
  ...
  const renderContent = () => {
    ...
    const groupedProducts = (filteredAndSortedProducts || []).reduce(
      (acc, product) => {
  ...
  return (
    <SafeAreaView style={styles.screenContainer}>
      ...
      {/* Filter Modal */}
      {showFilters && (
        <View style={styles.modalOverlay}>{renderFilterModal()}</View>
      )}
    </SafeAreaView>
  );
};
...
const styles = StyleSheet.create({
  screenContainer: {
    flex: 1,
    backgroundColor: "#F8E9DB", // Cream color
  ...
  contentContainer: {
    flex: 1,
    backgroundColor: "#ffffff", // Cream color
  },
  featuredHeaderRow: {
    flexDirection: "row",
